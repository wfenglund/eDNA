...
out <- dada2::filterAndTrim(primer1$Forward, primer1$FiltFs, 
                            primer1$Reverse, primer1$FiltRs, # comment out line if running single-ended
...
blastResY <- MetaBAnalysis::BlastParse(yAll, "y_test.out", threshold = 90)
...
SumRes2 <- function (blastRes, counts, taxGroup) 
{
    ...
    if (!taxGroup %in% names(taxGroupConv)) {
        ...
        sumAll <- aggregate(genesCounts[, 13:colStart], by = list(genesCounts$species), 
            FUN = sum)
        if (ncol(sumAll) > 2) {
        ...
        if (taxGroup == "Prokaryota" || taxGroup == "Eukaryota") {
            genesCountsFilt <- genesCounts[grepl(taxSel, blastRes$superkingdom), 
                ]
        }
        else if (taxGroup == "Plants" || taxGroup == "Fungi") {
        ...
        sumFilt <- aggregate(genesCountsFilt[, 13:colStart], 
            by = list(genesCountsFilt$species), FUN = sum)
        if (ncol(sumFilt) > 2) {
            sumFilt <- sumFilt[order(rowSums(sumFilt[, -1]), 
                decreasing = TRUE), ]
        }
        else {
...
allResults <- SumRes2(blastRes = blastResY, counts = yAll$counts, taxGroup = "Invertebrates") # Summarize a list of result objects

blastResY[blastResY$seq == 'Seq_1']
invClasses <- unique(blastResY$class[blastResY$phylum != "Chordata" & blastResY$kingdom == "Metazoa"])
invClasses <- invClasses[!is.na(invClasses)]
invString <- do.call(paste, c(as.list(invClasses), sep = "|"))
print(invClasses)
print(invString)
...

{
  ...
  ### Filter:
  groupNotes <- TrimCutoffs(total = 0.0, within = 0.0, minimum = 0, groupNotes) # trim species based on cutoffs
  
  ### Curate:
  groupNotes <- CurateHit(ref = 'hit1', note = '', absorb = 'hit2', latin = '', common = '', rmHit = TRUE, groupNotes)
