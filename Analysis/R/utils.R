...
#' Write Sequences Belonging To Each Sample To Individual Fasta Files
#'
#' @param countsObject object with columns of samples with counts and rows of sequences (where each row name is a specific sequence). This corresponds to yAll$counts generated by MakeDGEList()
#' @param fileSuffix string of text that will be placed at the end of the generated filenames: [sample name][fileSuffix].fa
#' 
WriteSeqsPerSample <- function(countsObject = yAll$counts, fileSuffix = "") {
  .WriteSeqs <- function(sampleName, countsDF, suffix = "") {
    countsDF <- countsDF[order(countsDF[, sampleName], decreasing = TRUE), ]
    fastaLine <- paste0(">", countsDF[countsDF[, sampleName] > 0, sampleName], "\n", rownames(countsDF[countsDF[, sampleName] > 0, ]), collapse = "\n")
    writeLines(fastaLine, paste0(sampleName, suffix, ".fa"))
    return("Done")
  }
  resultDF <- as.data.frame(countsObject)
  lapply(colnames(resultDF), .WriteSeqs, countsDF = resultDF, suffix = fileSuffix)
}
