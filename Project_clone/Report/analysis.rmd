...
# Parse BLAST results, by default only hits with both 90% identity as well as coverage are kept:
blastResY <- MetaBAnalysis::BlastParse(yAll, paste0("y_", projId, primer1$Prefix, ".out"), minIdentity = 90, minCoverage = 90)
saveRDS(blastResY, file = "blastResY.rds")
# blastResY2 <- MetaBAnalysis::BlastParse(yAll2, paste0("y_", projId, primer2$Prefix, ".out"), minIdentity = 90, minCoverage = 90)
...
# quickly write the sequences related to a species name to a file (1QSeqGetter
# and 1QSSplit) or blast the top three sequences related to a species and View the
# results in Rstudio (1CurationBLAST).
library(MetaBAnalysis)
library(annotate)
...
`1CurationBLAST` <- function(searchName, seqNumber = 3, blastResults = blastResY, curationDF = groupNotes) { # does work
...
`2CurationBLAST` <- function(searchName, seqNumber = 3) {`1CurationBLAST`(searchName = searchName, seqNumber = seqNumber, blastResults = blastResY2, curationDF = groupNotes2)}
...
sum(groupClean[1:3, -c(1, 2)]) / sum(groupClean[, -c(1, 2)]) # see what percentage of counts the top three species accounts for
...
### Generate ASV table:
primer1ASV <- GenerateASV()
primer1ASVGroup <- GenerateASV(filterByClass = TRUE) # fish classes is the default
# primer2ASV <- GenerateASV(blastObject = blastResY2, countsObject = yAll2)
# primer2ASVGroup <- GenerateASV(blastObject = blastResY2, countsObject = yAll2, filterByClass = TRUE) # fish classes is the default
writexl::write_xlsx(x = primer1ASV, path = paste0("Project_", projId, "_ASV_results_", primer1$Prefix, ".xlsx"))
writexl::write_xlsx(x = primer1ASVGroup, path = paste0("Project_", projId, "_ASV_results_", primer1$Prefix, "_only_group.xlsx"))
# writexl::write_xlsx(x = primer2ASV, path = paste0("Project_", projId, "_ASV_results_", primer2$Prefix, ".xlsx")
# writexl::write_xlsx(x = primer2ASVGroup, path = paste0("Project_", projId, "_ASV_results_", primer2$Prefix, "_only_group.xlsx"))
```
