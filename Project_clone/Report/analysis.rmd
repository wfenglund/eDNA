...
summaryAll <- aggregate(genesCounts[,13:dim(genesCounts)[2]], by = list(genesCounts$species), FUN = sum)
...
groupSummary <- aggregate(genesCountsGroup[,13:dim(genesCounts)[2]], by = list(genesCountsGroup$species), FUN=sum)
...
readRawReport <- rvest::read_html(x = "../path/to/report.html") # read the html report
rawDataTable <- as.data.frame(rvest::html_table(readRawReport, fill = TRUE)[4]) # select the relevant table

rawSeqs <- rawDataTable$Raw.reads
