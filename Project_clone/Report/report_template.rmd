...
header-includes:
   - \usepackage{geometry}
   - \usepackage{fancyhdr}
   - \usepackage[swedish,provide=*]{babel}
...
sampleCutoff <- "0,1" # Within sample cutoff
targetGroup <- "fisk" # group of organism focused on

primerName1 <- "12S"
primerName2 <- "16S" # if relevant

commonNo <- "3" # How many species that stood for [commonPercent] of all sequences
commonNo2 <- "3" # same as above but for second primer

seqPlotFigures <- "3-X" # Which figure numbers belong to the SeqPlots? e.G. "2" or "2-4" etc.
seqPlotFigures2 <- "4-X" # -"-

# References (uncomment the appropriate choice):
# primerVar <- list(citation = "[@miya2015mifish]", index = c(2)) # mifish
primerVar <- list(citation = "[@miya2015mifish;@wang2023vertu]", index = c(2, 13)) # mifish & 16s (fish)
# primerVar <- list(citation = "[@prie2021environmental;@zieritz2012identifying]", index = c(3, 11)) # ITS & 16s (mussels)
```
...
groupData <- readRDS("allResults.rds")$Clean
groupData <- cbind(groupData[, c(1, 2)], groupData[, -c(1, 2)][, order(as.numeric(gsub("[^0-9]+", "", colnames(groupData)[-c(1, 2)])))])
...
commonPercent <- paste0("~", round(sum(groupData[1:commonNo, -c(1, 2)]) / sum(groupData[, -c(1, 2)]) * 100)) # proportion of sequences that [commonNo] make up
...
if("allResults2.rds" %in% list.files(".")) { 
  ...
  commonPercent2 <- paste0("~", round(sum(allResults2$Clean[1:commonNo2, -c(1, 2)]) / sum(allResults2$Clean[, -c(1, 2)]) * 100)) # proportion of sequences that [commonNo2] make up
  speciesNo2 <- nrow(groupSpecies2) # Number of species found
  primerText2 <- paste0("För ", primerName2, " innehöll omkring ", primerPercent2, "% av sekvenserna förväntade primers och var av tillräckligt god kvalitet för att bibehållas i analysen. Av dessa tillhörde ~", groupPercent2, "% av sekvenserna ", targetGroup, ". Sekvenserna från ", targetGroup, " tillhörde ", speciesNo2, " olika arter (se figur 2 och ", seqPlotFigures2, " för relativa andelar sekvenser från varje art i varje prov) där de vanligaste ", commonNo2, " arterna stod för över ", commonPercent2, "% av sekvenserna från ", targetGroup, " (se figur 2).")
} else {
  groupSpecies2 <- c() # No species found, since no other primer
  primerText2 <- ""
}
...
tillverkaren. För analyserna amplifierades specifika DNA-fragment med 
primers `r primerVar$citation` och
PCR-reaktioner som optimerats för flerartsanalys med sekvensering (se Tabell 1).

```{r tab1, echo=FALSE}
library(kableExtra)
library(readxl)

# Do not edit this (generally):
primerInfo <- read_excel("primer_data.xlsx")
primerNames <- unlist(lapply(1:length(primerVar$index), function(x) {paste0(primerInfo[primerVar$index[x],1], footnote_marker_symbol(x))}))
primerInfo[primerVar$index, 1] <- primerNames
kable(primerInfo[primerVar$index, c(1:3,5)], "latex",
				 ...
				 kable_styling(latex_options = "hold_position", font_size = 8) %>%
			         footnote(symbol = apply(primerInfo[primerVar$index, 7], 2, function(x) {x})) # works fully for several primers
```
...
## Resultat
PCR-reaktioner genererade rena amplifieringar av förväntad storlek i båda
replikaten för alla prover och bedömdes därmed lämpliga för att skapa
...
groupCounts <- groupCounts[, rev(seq_along(groupCounts))] # reverse order of samples
if(ncol(groupCounts) > 1) { # if more than one sample, add a total of all samples to the plot
  groupCounts <- cbind(Totalt = rowSums(groupCounts), groupCounts)
}
figureSpecies <- data.frame(groupSpecies)$Svenska # change Svenska to Latin if needed

# Plot:
FractionPlot(data = groupCounts, species = figureSpecies, col = MetAlPipe:::color25, inset = c(-0.3, 0.01))
```
...
if("allResults2.rds" %in% list.files(".")) { 
  ...
  groupCounts2 <- groupCounts2[, rev(seq_along(groupCounts2))] # reverse order of samples
  if(ncol(groupCounts2) > 1) { # if more than one sample, add a total of all samples to the plot
    groupCounts2 <- cbind(Totalt = rowSums(groupCounts2), groupCounts2)
  }
  figureSpecies2 <- data.frame(groupSpecies2)$Svenska # change Svenska to Latin if needed
  
  # Plot:
  FractionPlot(data = groupCounts2, species = figureSpecies2, col = MetAlPipe:::color25, inset = c(-0.3, 0.01))
}
```
...
```{r fig5, fig.height=7, fig.width=14, echo=FALSE, fig.cap="Simpsons diversitetsindex för varje prov. Simpson (1 - D) ger sannolikheten att två slumpmässiga sekvenser dragna ur ett prov tillhör samma art. En högre siffra föreslår en högre diversitet inom det provet."}
...
# Format data:
countsMatrix <- as.matrix(t(groupData[,-c(1, 2)]))
...
## Projektinformation
Analysdata och resultat lagras tills vidare hos NRM och är open access. Vid eventuella
