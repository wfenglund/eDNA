...
# tables contatining the most frequent reads in forward and reverse reads are printed
...
# fastqc is run on every .gz (fastq) file in the Data directory
gz_files = [file for file in os.listdir('../Data/') if file.endswith('.gz')]
print(f'\nRunning FastQC on {len(gz_files)} files:')
for file in gz_files:
    print(f'File {(gz_files.index(file) + 1)} being processed...')
    subprocess.run(['fastqc ../Data/' + file + ' --outdir=./ --quiet'], shell = True)
print('Done.')
